...
class BaseProcessor(ABC):
	...
	@abstractmethod
	def process_before(self, dataset: List[Dict], language: str, task_mode: str, dataset_name: str, **kwargs) -> List[
		Dict]:
		"""
		processor prompt for model to generate result
		...
		return dataset: [{'task_id':'xxx','prompt':'xxx'}
		"""
		return dataset
	
	@abstractmethod
	def process_after(self, dataset: List[Dict], language: str, task_mode: str, dataset_name: str, **kwargs) -> List[
		Dict]:
		"""
		processor the model generation to evaluation
		param:dataset: [{'task_id':'xxx','prompt':'xxx','genereation':'xxx'}
		language:code language like python,java,cpp,js,go,rust
		task_mode:task mode for evaluation,define in DATASET_SUPPORT Variable in util.py
		dataset_name:evaluate dataset name,define in EVALUATE_DATASET Variable in util.py, the key is evaluation dataset name
		return dataset: [{'task_id':'xxx','prompt':'xxx','genereation':'xxx'}
		"""
		return dataset
