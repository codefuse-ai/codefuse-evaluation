...
from glm_chat.modeling_glm import GLMForConditionalGeneration
from glm_chat.tokenization_bailing import BailingTokenizer
# pip install atorch==1.4.1rc10 --no-deps --index=https://pypi.antfin-inc.com/artifact/repositories/simple/
...
class Bailingmoe_Processor(BaseProcessor):
    ...
    def __init__(self, **kwargs):
        ...
        self.model = GLMForConditionalGeneration.from_pretrained( self.path, device_map="auto", torch_dtype=torch.bfloat16 )
    ...
    def generate(self, promptlist: List[str] , language: str, task_mode: str, dataset_name: str, **kwargs) -> Union[List,str]:
        ...
        for prompt in promptlist:
            prompt = HUMAN_ROLE_START_TAG + prompt + BOT_TAG
            promptlist_new.append(prompt)

        promptlist = promptlist_new
        print(promptlist[0])
        print("===================================================================================")
        input_ids = self.tokenizer( promptlist, add_special_tokens=False )['input_ids']
        inputs = self.tokenizer.build_input_for_gen( input_ids, max_seq_len=self.model.config.max_sequence_length, max_out_len=4096 )
        generate_ids = self.model.generate( **inputs,
                                       max_new_tokens=4096,
                                       eos_token_id=self.tokenizer.eos_token_id,
                                       pad_token_id=self.tokenizer.pad_token_id )
        generations = self.tokenizer.batch_decode( generate_ids[:, inputs["input_ids"].shape[1]:], skip_special_tokens=True )
        return generations
