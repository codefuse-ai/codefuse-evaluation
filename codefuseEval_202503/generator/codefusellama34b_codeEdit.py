...
class CodeFuseCodeLlama34BProcessor(BaseProcessor):
	...
	def __init__(self, **kwargs):
		...
		if os.path.exists(self.path):
			...
			self.tokenizer = AutoTokenizer.from_pretrained(self.path, trust_remote_code=True, use_fast=False, legacy=False)
			...
			config, unused_kwargs = AutoConfig.from_pretrained(self.path, trust_remote_code=True, return_unused_kwargs=True)
			config_dict = config.to_dict()
			config_dict["use_flash_attn"] = 1
			config_dict["use_xformers"] = 1
			self.model = AutoModelForCausalLM.from_pretrained(self.path, config=config, device_map="auto", torch_dtype=torch.bfloat16, trust_remote_code=True, use_safetensors=False)
			print(self.tokenizer)
			print("=======tokenizer ids======")
