...
"""
mbxp 数据集 base 模型的后置处理，适用与百灵跟千问
"""
...
class MbxpBaseEnPostProcessor(BasePostProcessor):
    ...
    def post_process(self, data: Dict, language: str, task_mode: str, dataset_name: str, **kwargs) -> Dict:
        ...
        tmp = data.get("generation_ori")
        tmp = tmp.split("[BEGIN]\n")[1] if "[BEGIN]\n" in tmp else tmp
        tmp = tmp.split("[DONE]")[0] if "[DONE]" in tmp else tmp
        tmp = tmp.split("[D")[0] if "[D" in tmp else tmp
        if (tmp.startswith('\'') and tmp.endswith('\' \n')) or (tmp.startswith('\"') and tmp.endswith('\" \n')):
            tmp = "\n" + tmp[1: -3] + "\n"
            ...
            tmp = "\n" + tmp[2: -2] + "\n"
        elif (tmp.endswith('\'\n') and not tmp.startswith("\'")) or (tmp.endswith('\"\n') and not tmp.startswith("\"")):
            tmp = tmp[: -2] + "\n"
        elif (tmp.endswith('\' \n') and not tmp.startswith("\'")) or (
                tmp.endswith('\" \n') and not tmp.startswith("\"")):
            tmp = tmp[: -2] + "\n"
        real_generation = self.extract_code_from_markdown(tmp)
