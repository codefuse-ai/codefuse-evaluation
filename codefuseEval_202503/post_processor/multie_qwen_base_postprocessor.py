...
"""
multie 数据集 qwen 模型的后置处理，不适用于百灵模型
"""
...
class MultieQwenBasePostProcessor(BasePostProcessor):
    ...
    def post_process(self, data: Dict, language: str, task_mode: str, dataset_name: str, **kwargs) -> Dict:
        ...
        if text:
            ...
            text = ""
        text = text.split("<|file_sep|>")[0] if "<|file_sep|>" in text else text
        text = text.split("<|fim_middle|>")[0] if "<|fim_middle|>" in text else text
        if language.lower() == "cpp":
